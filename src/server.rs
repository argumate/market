...
struct Server {
    channel: sync::mpsc::Sender<(market::msgs::Request, futures::sync::oneshot::Sender<market::msgs::Response>)>
}
    ...
    fn call(&self, req: Request) -> Self::Future {
        ...
        match (req.method(), req.path()) {
            ...
            (&Method::Post, "/request") => {
                ...
                Box::new(req.body().concat2().map_err(|e| AppError::Hyper(e)).and_then(|b| {
                    ...
                    futures::future::result(tx.send(msg))
                        ...
                        .and_then(|_| {
                            on_reply.map_err(|_| AppError::Canceled).and_then(|market_reply| {
...
fn work_thread(
    mut market: Market,
    rx: sync::mpsc::Receiver<(market::msgs::Request, futures::sync::oneshot::Sender<market::msgs::Response>)>,
) -> Result<(), Error> {
    loop {
...
pub fn run_server(market: Market, addr_str: &str) -> Result<(), Error> {
    ...
    Http::new().bind(&addr, move || Ok(Server { channel: tx.clone() }))?.run_until(on_shutdown.map_err(|_| ()))?;
